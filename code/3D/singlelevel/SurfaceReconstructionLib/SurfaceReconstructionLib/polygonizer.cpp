...
namespace
{
  const int RES = 5; /* # converge iterations    */
  ...
  inline void converge (GPOINT* p1, GPOINT* p2, double v, 
								 ImplicitFunction* function, GPOINT* p)
  {
    int i = 0;
    GPOINT pos, neg;
    if (v < 0) {
  ...
  void vnormal (ImplicitFunction* function, GPOINT* point, GPOINT* n, double delta)
  {
    double f = function->eval(point->x, point->y, point->z);
  ...
  {	   /* parameters, function, storage */
    ...
    GPOINT start;		   /* start point on surface */
    ...
    inline TEST find (int sign, double x, double y, double z);
    
    inline int vertid (CORNER* c1, CORNER* c2);
    
    inline int dotet (CUBE* cube, int c1, int c2, int c3, int c4);
    
    inline int docube (CUBE* cube);

    inline int triangle (int i1, int i2, int i3)
    {
      TRIANGLE t;
      t.v0 = i1;
      t.v1 = i2;
  ...
  public:
      PROCESS(ImplicitFunction* _function,
          ...
          vector<TRIANGLE>& _gtriangles, double _xlength,double _ylength, double _zlength);
  ...
  inline int setcenter(vector<CENTERLIST>& table,
								int i, int j, int k)
  {
  ...
  inline CORNER* PROCESS::setcorner (int i, int j, int k)
  {
    ...
    c->i = i; c->x = start.x+((double)i-.5)*size*xlength;
    c->j = j; c->y = start.y+((double)j-.5)*size*ylength;
    c->k = k; c->z = start.z+((double)k-.5)*size*zlength;
  ...
  inline TEST PROCESS::find (int sign, double x, double y, double z)
  {
    int i;
    TEST test;
    double range = size;
    test.ok = 1;
    for (i = 0; i < 10000; i++) {
      test.p.x = x+range*(RAND()-0.5)*xlength;
      test.p.y = y+range*(RAND()-0.5)*ylength;
      test.p.z = z+range*(RAND()-0.5)*zlength;
  ...
  inline int PROCESS::vertid (CORNER* c1, CORNER* c2)
  {
    VERTEX v;
    NORMAL n;
    GPOINT a, b;
    ...
    //(*gnormals).push_back(n);			   /* save normal */
  ...
  inline int PROCESS::dotet (CUBE* cube, int c1, int c2, int c3, int c4)
  {
  ...
  inline int CUBETABLE::otherface (int edge, int face)
  {
	...
	inline CUBETABLE::CUBETABLE(): ctable(256)
  {
  ...
  inline int PROCESS::docube (CUBE* cube)
  {
    int index = 0;
    for (int i = 0; i < 8; i++) 
  ...
  PROCESS::PROCESS(ImplicitFunction* _function,
                ...
                double _xlength, double _ylength, double _zlength):
  ...
  void PROCESS::march(int mode, double x, double y, double z)
  {
    ...
    srand(1);
    in = find(1, x, y, z);//找到曲面内部的点（内正外负）
    out = find(0, x, y, z);//找到曲面外部的点（内正外负）
    if (!in.ok || !out.ok) 
...
void Polygonizer::march(bool tetra, double x, double y, double z)
{
	gvertices.clear();
	gnormals.clear();
	gtriangles.clear();
	PROCESS p(func, size, size/(float)(RES*RES), bounds, 
						gvertices, gnormals, gtriangles,this->xlength,this->ylength,this->zlength);
