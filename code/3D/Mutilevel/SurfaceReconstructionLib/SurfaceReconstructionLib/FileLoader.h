...
bool FileLoader::Load(const std::string& name) {
	...
	fclose(fp);
	// 创建八叉树
	cubetree=new CubeTree(minPoint, maxPoint, normals,10);
	cubetree->AddPoints(points, n);
	cubetree->Caculatecentriodandnormals(cubetree->root);
	octree=new Octree(minPoint, maxPoint,  10);
	octree->AddPoints(points, n); int leafcount = 0;
	rhohat = octree->Averageleaflength(octree->root,leafcount);
	rhohat = 0.75 * rhohat / leafcount;
	delete octree;
	cubetree->AddPoints(points, n);
	loaded = true;
...
void FileLoader::TryRelease() {
	...
	normals = NULL;
	if (cubetree != NULL) { delete cubetree; }
	cubetree = NULL;
}